...
void SceneTutorial::Init()
{
	...
	m_tutorialStep.resize(3);	// チュートリアルの段階数を入れる
	for (int i = 0; i < m_tutorialStep.size(); i++)
	{
		m_tutorialStep[i].m_step = i;										// チュートリアルの段階数
		m_tutorialStep[i].m_handle = LoadGraph(kTutorialHandlePath[i]);		// チュートリアル画像読み込み
		assert(m_tutorialStep[i].m_handle != -1);
		m_tutorialStep[i].m_checkPos = kTutorialCheckPos[i];				// チュートリアル画像のチェック座標セット
	}
	
	m_pMiniWindow->Init(m_pauseHandle);				// ポーズウィンドウ
	m_tutorialDisplay = m_tutorialStep[0].m_handle;	// チュートリアル段階の初期表示画像を挿入
}
...
std::shared_ptr<SceneBase> SceneTutorial::Update(Input& input)
{
	...
	if (input.IsTrigger(InputInfo::DebugStart)) {
		...
		{
			m_isPause = false;
			m_pMiniWindow->CloseMiniWindow();	// ポーズウィンドウを閉じる
	...
	if (m_isPause)
	{
		...
		{
			m_nextScene = nextScene::GameScene;	// ゲームシーン
			m_isNextSceneFlag = true;
		}
		if (input.IsTrigger(InputInfo::Back))
		{
			m_nextScene = nextScene::SelectScene;	// セレクトシーン
			m_isNextSceneFlag = true;
		}
