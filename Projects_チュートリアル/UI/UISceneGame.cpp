...
namespace {
	...
	const char* const kBulletNumberUI[10]{			// 残弾用UI画像
	...
	const char* const kScoreNumberUI[10]{	// スコア用UI画像
...
void UISceneGame::Draw()
{
	...
	DrawGraphF(kScoreUIPos[0].x, kScoreUIPos[0].y, m_scoreHandle.m_tenThousands, true);
	DrawGraphF(kScoreUIPos[1].x, kScoreUIPos[1].y, m_scoreHandle.m_thousands, true);
	DrawGraphF(kScoreUIPos[2].x, kScoreUIPos[2].y, m_scoreHandle.m_hundreds, true);
	DrawGraphF(kScoreUIPos[3].x, kScoreUIPos[3].y, m_scoreHandle.m_tens, true);
	DrawGraphF(kScoreUIPos[4].x, kScoreUIPos[4].y, m_scoreHandle.m_one, true);
...
void UISceneGame::Load()
{
	...
	// 残弾用UI画像の読み込み
	for (int i = 0; i < m_balanceBulletsNumberUIHandle.size(); i++)
	{
		m_balanceBulletsNumberUIHandle[i] = LoadGraph(kBulletNumberUI[i]);
		assert(m_balanceBulletsNumberUIHandle[i] != -1);
	}

	// スコア用UI画像の読み込み
	for (int i = 0; i < m_scoreNumberUIHandle.size(); i++)
	{
		m_scoreNumberUIHandle[i] = LoadGraph(kScoreNumberUI[i]);
		assert(m_scoreNumberUIHandle[i] != -1);
	}
}
...
void UISceneGame::End()
{
	...
	for (int i = 0; i < m_balanceBulletsNumberUIHandle.size(); i++)
	{
		DeleteGraph(m_balanceBulletsNumberUIHandle[i]);
	}

	// スコア用UI画像の削除
	for (int i = 0; i < m_scoreNumberUIHandle.size(); i++)
	{
		DeleteGraph(m_scoreNumberUIHandle[i]);
	}
}
...
{
	m_score.m_tenThousands = m_pScore->GetScore() / 10000;		// 10000の位
	m_score.m_thousands = (m_pScore->GetScore() % 10000) / 1000;// 1000の位
	m_score.m_hundreds = (m_pScore->GetScore() % 1000) / 100;	// 100の位
	m_score.m_tens = (m_pScore->GetScore() % 100) / 10;			// 10の位
	m_score.m_one = (m_pScore->GetScore() % 100) % 10;			// 1の位


	for (int i = 0; i < 10; i++)
	{
		// 10000の位
		if (m_score.m_tenThousands == i) {
			m_scoreHandle.m_tenThousands = m_scoreNumberUIHandle[i];
		}

		// 1000の位
		if (m_score.m_thousands == i) {
			m_scoreHandle.m_thousands = m_scoreNumberUIHandle[i];
		}

		// 100の位
		if (m_score.m_hundreds == i) {
			m_scoreHandle.m_hundreds = m_scoreNumberUIHandle[i];
		}

		// 10の位
		if (m_score.m_tens == i) {
			m_scoreHandle.m_tens = m_scoreNumberUIHandle[i];
		}

		// 1の位
		if (m_score.m_one == i) {
			m_scoreHandle.m_one = m_scoreNumberUIHandle[i];
		}
	}
}
...
void UISceneGame::SetUI_RemainingBulletsHandle(GunType type, int num)
{
	...
	if (hundred == 1)
	{
		balanceBullets[type].m_playerHundredsHandle = m_balanceBulletsNumberUIHandle[1];
	}
	else if (hundred == 0)
	{
	...
	for (int i = 0; i < 10; i++)
	{
		if (ten == i)
		{
			balanceBullets[type].m_playerTensHandle = m_balanceBulletsNumberUIHandle[i];
		}

		if (one == i) {
			balanceBullets[type].m_playerOneHandle = m_balanceBulletsNumberUIHandle[i];
		}
	}
}