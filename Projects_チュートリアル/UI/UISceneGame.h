...
class UISceneGame
{
	...
	struct NowScoreHandle {
		int m_tenThousands = 0;	// スコアの10000の位
		int m_thousands = 0;	// スコアの1000の位
		int m_hundreds = 0;		// スコアの100の位
		int m_tens = 0;			// スコアの10の位
		int m_one = 0;			// スコアの1の位
	}m_scoreHandle;	// スコアの値(画像)
...
private:
	...
	std::array<int, 10>m_balanceBulletsNumberUIHandle{};// 残弾表示UI用ハンドル
	std::array<int, 10>m_scoreNumberUIHandle{};			// 残弾表示UI用ハンドル
